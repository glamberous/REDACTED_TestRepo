...
half4 loadBoneMatrixTexture(Texture2DArray<float4> animatedBoneMatrices, int textureIndex, int frameIndex, int boneIndex, int i)
{
    return animatedBoneMatrices.Load(int4(textureIndex, (boneIndex * 3) + i, frameIndex, 0));
}
...
void calculateFrameValues(half3 position, half3 normal, half3 tangent,
    Texture2DArray<float4> animatedBoneMatrices, 
    half2 boneWeights[6], int textureIndex, int frameIndex, 
    out half3 positionOut, out half3 normalOut, out half3 tangentOut)
{
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        if(boneWeight != 0)
        {
            ...
            half4 m0 = loadBoneMatrixTexture(animatedBoneMatrices, textureIndex, frameIndex, boneIndex, 0); 
            half4 m1 = loadBoneMatrixTexture(animatedBoneMatrices, textureIndex, frameIndex, boneIndex, 1); 
            half4 m2 = loadBoneMatrixTexture(animatedBoneMatrices, textureIndex, frameIndex, boneIndex, 2); 
...
void AnimateBlend_half(half3 position, half3 normal, half3 tangent, 
    half3x4 uvs, Texture2DArray<float4> animatedBoneMatrices, int textureIndex, half4x3 animationState,
    out half3 positionOut, out half3 normalOut, out half3 tangentOut) 
{
    ...
    for(int blendIndex = 0; blendIndex < 4; blendIndex++)
    {
        ...
        if(blendFactor > 0)
        {
            ...
            calculateFrameValues(position, normal, tangent, animatedBoneMatrices, boneWeights, textureIndex, frameIndex, 
                posOutBefore, normalOutBefore, tangentOutBefore);
            calculateFrameValues(position, normal, tangent, animatedBoneMatrices, boneWeights, textureIndex, frameIndex + 1,
